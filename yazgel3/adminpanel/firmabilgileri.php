...
<body>
    ...
    if (isset($_POST["cmdaddnew"])) {
        $sql = "INSERT INTO firmalar (firma_id, firma_adi, logo)
                VALUES
                (:firma_id, :firma_adi, :logo)";

        $stmt = $baglanti->prepare($sql);

        $stmt->bindParam(':firma_id', $_POST["firma_id"]);
        $stmt->bindParam(':firma_adi', $_POST["firma_adi"]);
        $stmt->bindParam(':logo', $_POST["logo"]);


        if ($stmt->execute()) {
            echo "<script type= 'text/javascript'>alert('Kayıt başarıyla güncellendi');
                window.location.replace('firmabilgileri.php');</script>";
        } else {
    ...
    if ($editemployee == '') {
        ...
        $sql = "SELECT * FROM firmalar WHERE firma_id = :firma_id";
        $stmt = $baglanti->prepare($sql);
        $stmt->bindParam(':firma_id', $editemployee);
        $stmt->execute();
    ...
    if (isset($_POST["cmdelete"])) {
        $sql = "DELETE FROM firmalar WHERE firma_id='$deleteemployee'";
    ...
    if (isset($_POST["cmdedit"])) {
        ...
        $sql = "UPDATE firmalar SET firma_id=:firma_id, firma_adi=:firma_adi, logo=:logo WHERE firma_id=:firma_id";
    ...
    <div class="container">
        ...
        <div class="table-wrapper">
            ...
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Firma <b>Ekleme</b></h2>
                    </div>
                    <div class="col-sm-6">
                        <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i
                                class="material-icons">🍳</i> <span>Firma Ekle</span></a>
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php
                    $urunsor = $baglanti->prepare("SELECT * FROM firmalar");
                    $urunsor->execute();
                    while ($uruncek = $urunsor->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <?PHP echo $uruncek['firma_adi'] ?>
                            </td>
                            <td>
                                <?PHP echo $uruncek['logo'] ?>
                            </td>


                            <td>
                                <a href="?editemployee=<?php echo $uruncek['firma_id']; ?>" class="edit"
                                    data-toggle="modal"><i class="material-icons" data-toggle="tooltip"
                                        title="Edit"></i></a>
                                <a href="?deleteemployee=<?php echo $uruncek['firma_id']; ?>" class="delete"
                                    data-toggle="modal"><i class="material-icons" data-toggle="tooltip"
                                        title="Delete"></i></a>
                            </td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form name="frmedit" action="" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title">Firma Düzenleme</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>firma_adi</label>
                                <input type="text" name="firma_adi" value="<?php echo $firma_adi; ?>"
