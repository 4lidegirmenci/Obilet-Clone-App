...
<body>
    ...
    if (isset ($_POST["cmdaddnew"])) {
        $sql = "INSERT INTO biletler (bilet_id, kullanici_id, sefer_id, koltuk_id, bilet_tarihi, odeme_tipi, pnr_kod)
                VALUES
                (:bilet_id, :kullanici_id, :sefer_id, :koltuk_id, :bilet_tarihi, :odeme_tipi, :pnr_kod)";

        $stmt = $baglanti->prepare($sql);

        $stmt->bindParam(':bilet_id', $_POST["bilet_id"]);
        ...
        $stmt->bindParam(':bilet_tarihi', $_POST["bilet_tarihi"]);
        $stmt->bindParam(':odeme_tipi', $_POST["odeme_tipi"]);
        $stmt->bindParam(':pnr_kod', $_POST["pnr_kod"]);

        if ($stmt->execute()) {
            echo "<script type= 'text/javascript'>alert('Kayıt başarıyla güncellendi');
                window.location.replace('rezarvasyonbilgileri.php');</script>";
        } else {
    ...
    if ($editemployee == '') {
        ...
        $sql = "SELECT * FROM biletler WHERE bilet_id = :bilet_id";
        $stmt = $baglanti->prepare($sql);
        $stmt->bindParam(':bilet_id', $editemployee);
        $stmt->execute();
    ...
    if (isset ($_POST["cmdelete"])) {
        $sql = "DELETE FROM biletler WHERE bilet_id='$deleteemployee'";
    ...
    if (isset ($_POST["cmdedit"])) {
        ...
        $bilet_tarihi = $_POST["bilet_tarihi"];
        $odeme_tipi = $_POST["odeme_tipi"];
        $pnr_kod = $_POST["pnr_kod"];
        $sql = "UPDATE biletler SET bilet_id=:bilet_id, kullanici_id=:kullanici_id, sefer_id=:sefer_id, koltuk_id=:koltuk_id, 
        bilet_tarihi=:bilet_tarihi, odeme_tipi=:odeme_tipi, pnr_kod=:pnr_kod WHERE bilet_id=:bilet_id";
        ...
        $stmt->bindParam(':bilet_tarihi', $bilet_tarihi);
        $stmt->bindParam(':odeme_tipi', $odeme_tipi);
        $stmt->bindParam(':pnr_kod', $pnr_kod);
        $stmt->bindParam(':bilet_id', $editemployee);
    ...
    <div class="container">
        ...
        <div class="table-wrapper">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i
                                class="material-icons">🍳</i> <span>Rezervasyon Ekle</span></a>
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php
                    $urunsor = $baglanti->prepare("SELECT * FROM biletler");
                    $urunsor->execute();
                    while ($uruncek = $urunsor->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <?PHP echo $uruncek['bilet_tarihi'] ?>
                            </td>
                            <td>
                                <?PHP echo $uruncek['odeme_tipi'] ?>
                            </td>

                            <td>
                                <a href="?editemployee=<?php echo $uruncek['bilet_id']; ?>" class="edit"
                                    data-toggle="modal"><i class="material-icons" data-toggle="tooltip"
                                        title="Edit"></i></a>
                                <a href="?deleteemployee=<?php echo $uruncek['bilet_id']; ?>" class="delete"
                                    data-toggle="modal"><i class="material-icons" data-toggle="tooltip"
                                        title="Delete"></i></a>
                            </td>
                    ...
                    <form action="" method="post">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label>bilet_id</label>
                                <input type="text" name="bilet_id" class="form-control" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>bilet_tarihi</label>
                                <input type="text" name="bilet_tarihi" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>odeme_tipi</label>
                                <input type="text" name="odeme_tipi" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>pnr_kod</label>
                                <input type="text" name="pnr_kod" class="form-control" required>
                            </div>
                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form name="frmedit" action="" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title">Rezervasyon Düzenleme</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group">
                                <label>bilet_id</label>
                                <input type="text" name="bilet_id" value="<?php echo $bilet_id; ?>" class="form-control"
                                    required>
                            </div>
                            <div class="form-group">
                                <label>kullanici_id</label>
                            ...
                            <div class="form-group">
                                <label>bilet_tarihi</label>
                                <input type="text" name="bilet_tarihi" value="<?php echo $bilet_tarihi; ?>"
                                    class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>odeme_tipi</label>
                                <input type="text" name="odeme_tipi" value="<?php echo $odeme_tipi; ?>"
                                    class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>pnr_kod</label>
                                <input type="text" name="pnr_kod" value="<?php echo $pnr_kod; ?>" class="form-control"
                                    required>
                            </div>
                        </div>
                        <div class="modal-footer">
